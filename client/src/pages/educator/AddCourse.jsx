      ...
      <div className='h-screen overflow-scroll flex flex-col items-start
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 max-w-md w-full text-gray-500">
          ...
          <div>
            ...
            <img src={assets.dropdown_icon} onClick={()=>handleChapter('toggle',chapter.chapterId)}
            width={14} alt="" className={` mr-2 cursor-pointer transition-all ${chapter.collapsed &&
            "-rotate-90"}`} />
            ...
            src={assets.cross_icon} className="cursor-pointer" />
            </div>
            {!chapter.collapsed && (
              <div className="p-4">
                {chapter.chapterContent.map((lecture,lectureIndex)=>(
                  <div key={lectureIndex} className="flex justify-between items-center mb-2">
                    <span>{lectureIndex + 1} {lecture.lectureTitle} - {lecture.lectureDuration} mins - <a href={lecture.lectureUrl}
                      target="_blank" className="text-blue-500">Link</a> - {lecture.isPreviewFree ? 'Free Preview' : 'Paid' }
                    </span>
                    <img src={assets.cross_icon} onClick={() => handleLecture('remove',chapter.chapterId,lectureIndex)}
                     className="cursor-pointer"/>
                  </div>
                ))}
                <div className="inline-flex bg-gray-100 p-2 rounded cursor-pointer mt-2"
                 onClick={()=>handleLecture('add',chapter.chapterId)}>+ Add Lecture</div>
              </div>
            )}
            </div>
            ))}
              ...
              showPopup && (
                ...
                bg-gray-800 bg-opacity-80">
                  <div className="bg-white text-gray-700 p-4 rounded relative w-full max-w-80">
                    ...
                    <img src={assets.cross_icon} onClick={() => setShowPopup(false)} 
